...
export default defineEventHandler(async (event) => {
  ...
  // alongside vault/search here. /api/settings/*, /api/trash/* (Phase 5) and
  // /api/templates/* (Phase 6) join the same list for the same reason.
  // /api/auth/* stays unprotected as a whole (setup/login/logout must be
  // reachable pre-session) except the two account-mutation routes, which
