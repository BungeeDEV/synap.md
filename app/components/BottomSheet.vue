...
<template>
  <Transition enter-active-class="transition duration-150 ease-out" leave-active-class="transition duration-100 ease-in" enter-from-class="opacity-0" leave-to-class="opacity-0">
    <div v-if="open" class="fixed inset-0 z-50 backdrop-blur-md bg-black/40" @click="emit('close')" />
  </Transition>
  ...
  >
    <div
      v-if="open"
      role="dialog"
      class="fixed inset-x-0 bottom-0 z-50 flex max-h-dialog flex-col rounded-t-xl border-t border-border-strong bg-surface-1 pb-safe-b shadow-float"
      @click.stop
    >
