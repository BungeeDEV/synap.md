...
<template>
  <div v-if="!loading && backlinks.length" class="shrink-0 border-t border-border px-4 py-2">
    ...
    <ul class="flex flex-wrap gap-2">
      <li v-for="link in backlinks" :key="link.path">
        <button
          type="button"
          class="rounded-full bg-surface-2 px-3 py-1 text-sm text-content-secondary transition-colors duration-150 hover:bg-white/[0.04] hover:text-content-primary"
