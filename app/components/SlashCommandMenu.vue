...
<template>
  <div
    v-if="menu"
    class="fixed z-50 min-w-48 rounded-xl border border-border-strong bg-surface-1 py-1 text-sm text-content-primary shadow-float"
  ...
  >
    <ul v-if="filtered.length">
      <li v-for="(command, i) in filtered" :key="command.id">
        <button
          type="button"
          class="flex w-full items-center gap-2 px-3 py-1.5 text-left transition-colors duration-150"
          :class="i === menu.selectedIndex ? 'bg-surface-2' : 'hover:bg-white/[0.04]'"
          @click="select(i)"
          @mouseenter="menu.selectedIndex = i"
        >
          <component :is="command.icon" class="h-4 w-4 shrink-0 text-content-tertiary" stroke-width="1.5" />
          {{ command.label }}
        </button>
      </li>
    </ul>
    <p v-else class="px-3 py-1.5 text-content-tertiary">
      Keine Treffer
    </p>
  </div>
</template>
