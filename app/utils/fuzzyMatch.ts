...
/** Recursively collects every file node from the vault tree, in tree order. */
export function flattenFiles(nodes: VaultTreeNode[]): FileEntry[] {
  const files: FileEntry[] = []

  for (const node of nodes) {
    if (node.type === 'file') {
      files.push({ path: node.path, title: titleFromPath(node.name) })
